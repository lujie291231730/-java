	   ...
	   public int GetAuth()
	    {
	      ...
	      if (this.abrowse) auth += 1;

	      if (this.anew) auth += 2;

	      if (this.aedit) auth += 4;

	      if (this.adelete) auth += 8;

	      if (this.aprint) auth += 16;

	      if (this.aexport) auth += 32;

	      if (this.adeal) auth += 64;

	      if (this.avalid) auth += 128;
	      
	      if (this.ainvalid) auth += 256;
	      
	      if (this.acheck) auth += 512;
	      
	      if (this.auncheck) auth += 1024;
	      
	      if (this.aupload) auth += 2048;
	      
	      if (this.aspecial01) auth += 4096;
	      
	      if (this.aspecial02) auth += 8192;
	      
	      if (this.aspecial03) auth += 16384;
	      
	      if (this.asubmit) auth += 32768;
	    ...
	    public String GetAuthString()
	    {
	      ...
	      return StringUtils.LeftFill(sauth, 7);
	    }
