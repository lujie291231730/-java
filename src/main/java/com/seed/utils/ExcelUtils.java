...
import com.seed.entity.system.SysPmt;
		...
		try {
			...
			for (T t : lists) {
				...
				for (ExportColumn ec : cols) {
					System.out.println(ec.getDataindex());
			...
			if ((ou != null) && !ToolUtils.StringIsEmpty(ou.getUser().getUserid())) {
				if (cols.size() >= 3) {
					...
					if (es.isHasexportdate()) {
						...
						sheet.addCell(new jxl.write.Label(2, nowrow, "制表日期：" + ToolUtils.GetHMFmtDate(ToolUtils.GetNowDate()), wcfright));	
					}
				}
				else {
					String ed = "";
