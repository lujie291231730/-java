...
public class WsApp implements BaseBean {
		...
		if (StringUtils.StringIsEmpty(this.getAppid()))
    {
      ...
      rtn = true;
		...
		if (StringUtils.StringIsEmpty(this.getAppname()))
    {
      msg.setErrmsg("服务商名称不能为空！" + StringUtils.GetNewLines());
      rtn = true;
    }
		
		if (StringUtils.StringIsEmpty(this.getAppkey()))
    {
      msg.setErrmsg("密钥不能为空！" + StringUtils.GetNewLines());
      rtn = true;
    }
