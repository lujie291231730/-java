...
public class WsVisit implements BaseBean {
	...
	public boolean OnBeforeSave(ErrorMsg msg) {
		...
		if (StringUtils.StringIsEmpty(this.getAppid()))
    {
      msg.setErrmsg("服务商编号不能为空！" + StringUtils.GetNewLines());
      rtn = true;
    }
		...
		if (StringUtils.StringIsEmpty(this.getVisitfunc()))
    {
      msg.setErrmsg("访问函数不能为空！" + StringUtils.GetNewLines());
      rtn = true;
    }
	...
	public String[] OnProperties() {
		return new String[] {"访问序号:visitid", "服务商编号:appid", "访问日期:visitdate", "访问函数:visitfunc", 
				"访问地址:visitaddress", "备注:visitcontent"};
	}
